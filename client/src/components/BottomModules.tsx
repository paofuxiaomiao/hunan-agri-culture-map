...
export default function BottomModules() {
  return (
    <div className="w-full border-t border-gold/15" style={{ background: 'linear-gradient(180deg, #faf8f2 0%, #f6f1e8 100%)' }}>
      {/* Gold thread divider */}
      <div className="gold-thread" />
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
        {/* Theme Routes - 3 cols */}
        <div className="lg:col-span-3 p-4 border-r border-gold/10">
              ...
              <div key={route.id} className="group cursor-pointer">
                <div className="flex gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-xs font-semibold text-foreground truncate group-hover:text-gold-dark transition-colors">{route.name}</h4>
            ...
            {timelineEvents.map((event, index) => (
              <div key={event.id} className="flex items-start gap-2 group cursor-pointer">
                <div className="flex flex-col items-center">
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-gold/15 to-gold/5 border border-gold/25 flex items-center justify-center text-xs group-hover:border-gold/50 group-hover:shadow-sm transition-all">
                    {event.icon}
                  </div>
                ...
                <div className="flex-1 pb-0.5">
                  <h4 className="text-xs font-medium text-foreground group-hover:text-gold-dark transition-colors">{event.title}</h4>
